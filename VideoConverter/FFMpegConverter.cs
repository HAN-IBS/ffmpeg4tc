namespace VideoConverter
{
    ...
    public class FFMpegConverter
    {
        ...
        protected string ComposeFFMpegCommandLineArgs(string inputFile, string inputFormat, string outputFile, string outputFormat, ConvertSettings settings)
        {
            ...
            return string.Format(" -loglevel {4} {0} -i {1} {2} -y {3}", new object[] { builder.ToString(), this.CommandArgParameter(inputFile), outputArgs.ToString(), this.CommandArgParameter(outputFile), this.LogLevel });
        }
        ...
        protected void ComposeFFMpegOutputArgs(StringBuilder outputArgs, string outputFormat, OutputSettings settings)
        {
            if (settings != null)
            {
                ...
                if (settings.AudioBitRate != null)
                    outputArgs.AppendFormat(" -b:a {0}", settings.AudioBitRate);
                if (settings.VideoBitRate!= null)
                    outputArgs.AppendFormat(" -b:v {0}", settings.VideoBitRate);
        ...
        public void ConcatMedia(string[] inputFiles, string outputFile, string outputFormat, ConcatSettings settings)
        {
			...
			if (File.Exists(ifList)) File.Delete(ifList);
